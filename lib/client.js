	...
	factory: (require) => {
		...
		function ensureStyle() {
			...
			el.textContent = [
				...
				".wf-contextBtn { background: var(--dsw-alias-state-business-primary, #3b82f6); border: 0; color: #fff; border-radius: 5px; padding: 4px 9px; cursor: pointer; font: inherit; font-size: 12px; white-space: nowrap; }",
				".wf-contextBtn:hover { filter: brightness(1.08); }",
				".wf-contextBtn:disabled { opacity: .65; cursor: default; filter: none; }",
				// Segmented tabs (文件 | Git) — both segments are always present so the
				// control keeps a constant width; only the active tint moves. This
				// avoids the layout jump a single label-swapping toggle caused.
				".wf-seg { display: inline-flex; flex: 0 0 auto; border: 1px solid var(--dsw-alias-border-l2, rgba(140,140,140,.24)); border-radius: 6px; }",
				".wf-segbtn {",
				"  position: relative; display: inline-flex; align-items: center; justify-content: center; gap: 4px;",
				"  padding: 3px 10px; border: 0; background: transparent; cursor: pointer; font: inherit; font-size: 12px;",
				"  line-height: 1.4; white-space: nowrap; color: var(--dsw-alias-label-tertiary, inherit);",
				"}",
				".wf-segbtn:first-child { border-radius: 5px 0 0 5px; }",
				".wf-segbtn:last-child { border-radius: 0 5px 5px 0; }",
				".wf-segbtn + .wf-segbtn { border-left: 1px solid var(--dsw-alias-border-l2, rgba(140,140,140,.24)); }",
				".wf-segbtn:hover { background: var(--dsw-alias-interactive-bg-hover, rgba(140,140,140,.12)); color: var(--dsw-alias-label-primary, inherit); }",
				".wf-segbtn.active { background: var(--dsw-alias-interactive-bg-hover, rgba(140,140,140,.22)); color: var(--dsw-alias-label-primary, inherit); }",
				// Change-count badge, absolutely positioned so it never widens the tab.
				".wf-segbadge {",
				"  position: absolute; top: -6px; right: -7px; min-width: 15px; height: 15px; padding: 0 4px;",
				...
				"  box-shadow: 0 0 0 1.5px var(--dsw-alias-bg-layer-1, #1e1e1e);",
				"}",
		...
		/** Build the Markdown snippet copied by Add to context. */
		function contextSnippet(file, root) {
			...
			const selected = selection.replace(/\r\n/g, "\n");
			let start = 1;
			let end = Math.max(1, content.split("\n").length);
			let snippet = content;
			if (selected.trim()) {
				const offset = content.indexOf(selected);
				if (offset >= 0) {
					start = content.slice(0, offset).split("\n").length;
					end = start + selected.split("\n").length - 1;
					snippet = selected;
				}
			}
			const absolute = String(file.path || file.name || "");
			const rootText = String(root || "").replace(/[\\/]+$/, "");
			const relative = rootText && absolute.startsWith(rootText + "/") ? absolute.slice(rootText.length + 1) : (file.name || absolute);
			const lang = langFromName(file.name || relative) || "";
			return "File: " + relative + "\nLines: " + start + "-" + end + "\n\n```" + lang + "\n" + snippet + "\n```";
		}

		async function copyToContext(file, root) {
			const text = contextSnippet(file, root);
		...
		function ContentView(props) {
			...
			const header = h(
				...
				!file.binary
					? h("button", {
						className: "wf-contextBtn",
						title: "复制当前文件或选中的代码片段到上下文",
						onMouseDown: (event) => event.preventDefault(),
						onClick: () => copyToContext(file, root).then(() => {
							setCopied(true);
							window.setTimeout(() => setCopied(false), 1600);
						}).catch((e) => setError(e.message)),
					}, copied ? "已添加" : "Add to context")
					: null,
				hasGit
					? h(
							"span",
		...
		function FilesView(props) {
			...
			return h(
				...
				h(
					"div",
					{ style: headerStyle },
					// Segmented tabs at the far left, switching the pane between the file
					// tree and the VS Code-style Git panel. Both tabs are always shown
					// (constant width — no layout jump), and a change-count badge floats
					// on the Git tab. Only shown when the workspace is a repo.
					hasGit
						? h(
								"div",
								{ className: "wf-seg" },
								h(
									"button",
									{ className: "wf-segbtn" + (gitMode ? "" : " active"), onClick: () => setGitMode(false), title: "文件树" },
									"📁 文件",
								),
								h(
									"button",
									{ className: "wf-segbtn" + (gitMode ? " active" : ""), onClick: () => setGitMode(true), title: "Git 源代码管理" },
									"⎇ Git",
									Object.keys(statusMap).length ? h("span", { className: "wf-segbadge" }, String(Object.keys(statusMap).length)) : null,
								),
							)
						: null,
